...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    ...
    static const float VOLTAGE_FACTOR_1 = 18.9f;
    ...
    static const float VOLTAGE_OFFSET_2 = 1.60f + 0.1f;
    ...
    if(hadc==&hadc2)
    {
        ...
        // 使用预计算常量减少计算量
        current_ac[0][cnt2] = (sum4 * ADC2_SCALE - CURRENT_OFFSET) * CURRENT_FACTOR; // ac1_i
        current_ac[2][cnt2] = (sum2 * ADC2_SCALE - CURRENT_OFFSET) * CURRENT_FACTOR; // ac3_i
        voltage_ac[2][cnt2] = (sum1 * ADC2_SCALE - VOLTAGE_OFFSET_2) * VOLTAGE_FACTOR_1;
        voltage_ac[1][cnt2] = (sum3 * ADC2_SCALE - VOLTAGE_OFFSET_3) * VOLTAGE_FACTOR_2;
        if (++cnt2 >= SAMPLING_2_LENS)
        {
            arm_rms_f32((const float *)voltage_ac[1], SAMPLING_2_LENS, &data->volatage[1]);
            arm_rms_f32((const float *)voltage_ac[2], SAMPLING_2_LENS, &data->volatage[2]);
            arm_rms_f32((const float *)current_ac[0], SAMPLING_2_LENS, &data->current[0]);
            arm_rms_f32((const float *)current_ac[2], SAMPLING_2_LENS, &data->current[2]);
            cnt2 = 0;
        }
    }
    else if (hadc==&hadc3)
    {
        ...
        // 使用预计算常量
        voltage_ac[0][cnt3] = (sum1 * ADC3_SCALE - VOLTAGE_OFFSET_1) * VOLTAGE_FACTOR_1;
        current_ac[1][cnt3] = (sum3 * ADC3_SCALE - CURRENT_OFFSET) * CURRENT_FACTOR;
        if (++cnt3 >= SAMPLING_3_LENS)
        {
            arm_rms_f32((const float *)voltage_ac[0], SAMPLING_3_LENS, &data->volatage[0]);
            data->volatage[0] += 0.075;
            arm_rms_f32((const float *)current_ac[1], SAMPLING_3_LENS, &data->current[1]);
            cnt3 = 0;
        }
    }
}
