...
namespace kalman_filter_N
{
    ...
    /* kalman_filter_c::kalman_filter_c(KalmanFilter_t *kf, uint8_t xhatSize, uint8_t uSize, uint8_t zSize)
    {
        kalman_filter_init(kf, xhatSize, uSize, zSize);
    }
    kalman_filter_c::~kalman_filter_c()
    {
    } */
    ...
    void kalman_filter_xhatMinus_update(KalmanFilter_t *kf)
    {
        if (!kf->SkipEq1)
        {
            if (kf->uSize > 0)
    ...
    // 预测更新
    // 2. P'(k) = A·P(k-1)·AT + Q
    void kalman_filter_Pminus_update(KalmanFilter_t *kf)
    {
        if (!kf->SkipEq2)
        {
    ...
    void kalman_filter_xhat_update(KalmanFilter_t *kf)
    {
        if (!kf->SkipEq4)
        {
     ...
     *  只需
     * */
    first_order_kalman_filter_c::first_order_kalman_filter_c(float T_Q, float T_R)
    {
        X_last = (float)0;
        P_last = 0;
        Q = T_Q;
     ...
     *         T_R:测量噪声协方差
     * @retval none
     * @attention R固定，Q越大，代表越信任侧量值，Q无穷代表只用测量值x'x
     *		       	反之，Q越小代表越信任模型预测值，Q为零则是只用模型预测
     */
    void first_order_kalman_filter_c::first_order_kalman_filter_init(float T_Q, float T_R)
    {
        X_last = (float)0;
        P_last = 0;
        Q = T_Q;
    ...
    float first_order_kalman_filter_c::first_order_kalman_filter_calc(float data)
    {
        X_predict = X_last;                                   // 计算当前时刻的预测结果(1)  x(k|k-1) = A*X(k-1|k-1) + B*U(k) + W(K)
        P_predict = P_last + Q;                               // 计算当前时刻预测结果的协方差(2)    p(k|k-1) = A*p(k-1|k-1)*A' + Q
        kalman_gain = P_predict / (P_predict + R);            // 计算klaman增益(3) kg(k) = p(k|k-1)*H' / (H*p(k|k-1)*H'+R)
        X_now = X_predict + kalman_gain * (data - X_predict); // 计算当前时刻的最优结果(4)    x(k|k) = X(k|k-1) + kg(k)*(Z(k) - H*X(k|k-1))
        P_now = (1 - kalman_gain) * P_predict;                // 计算当前时刻最优结果的协方差(5)    p(k|k) = (I-kg(k)*H) * P(k|k-1)
