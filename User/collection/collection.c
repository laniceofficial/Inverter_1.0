...
uint16_t ask_adc_data[ROW_LEN] = {0};
...
collect_data_t *collection_init(void)
{
    ...
    HAL_ADC_Start_DMA(&hadc3, (uint32_t *)adc3_data, 8);
    ...
    ask_init(&ask_com, ask_adc_data, ROW_LEN);
    return data;
}
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    ...
    //   collection_stop();
    // 预定义常量以提高计算效率
    static const float ADC2_SCALE = VOL_REF / (ROW_LEN * ADC_MAX_VALUE);
    static const float ADC3_SCALE = VOL_REF / (ROW_LEN * ADC_MAX_VALUE);
    ...
    if (hadc == &hadc2)
    {
        ...
        sum_t = sum1 / ROW_LEN;
        sour_v_f = Recursive_ave_filter(&Vfilter2, sum1, filter_n) * ADC2_SCALE;
        sum1 = LPF_Update(&FOFilter, sum1);
      
        // base_v = (sum2 * ADC2_SCALE);
        // float now_base = base_v/ (ROW_LEN * ADC_MAX_VALUE);
        sour_v = (sum1 * ADC2_SCALE);
        // cal_v = (sum1 * ADC2_SCALE - VOLTAGE_OFFSET_2 + (1.112 - base_v)) *
        // factor;
        cal_without_base_v = (sum1 * ADC2_SCALE - VOLTAGE_OFFSET_2) * factor;
        ask_adc_data[psc_cnt] = sum1 / ROW_LEN;
        psc_cnt++;
        if (psc_cnt >= ROW_LEN)
        {
            psc_cnt = 0;
    ...
    else if (hadc == &hadc3)
    {
        ...
        // voltage_ac[0][cnt3] =
        //     ((float)sum1 / ROW_LEN / ADC_MAX_VALUE * VOL_REF - 1.60) * 18.9f;
        // current_ac[1][cnt3] = ((float)sum3 / ROW_LEN / ADC_MAX_VALUE * VOL_REF
        // - 2.49) *
        //                       8; // 使用预计算常量
        // voltage_ac[0][cnt3] =
        //     (sum1 * ADC3_SCALE - VOLTAGE_OFFSET_1) * VOLTAGE_FACTOR_1;
        // current_ac[1][cnt3] = (sum3 * ADC3_SCALE - CURRENT_OFFSET) *
        // CURRENT_FACTOR; if (++cnt3 >= SAMPLING_3_LENS) {
