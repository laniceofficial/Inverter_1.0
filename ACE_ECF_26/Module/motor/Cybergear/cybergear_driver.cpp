...
using namespace Motor_n::CyberGearMotor_n;

CyberGearDriver_c* CyberGearDriver_c::MI_motor_instance_head = nullptr;
uint8_t CyberGearDriver_c::MI_num_ = 0;

//函数声明
#ifdef STM32H723xx
void MiMotorReceiveCallback(BSP_n::Fdcan_c* register_instance);
#else
void MiMotorReceiveCallback(BSP_n::Can_c* register_instance);
#endif
...
CyberGearDriver_c::CyberGearDriver_c(MotorBaseDef_n::Motor_Base_Config_t motor_config) : MotorBase_c(motor_config)
{
    this->next_ = nullptr;
    //记录下数量
    this->MI_num_++;
    this->MiMotorSetMechPosition2Zero();

    if(MI_motor_instance_head == nullptr)
    {
        MI_motor_instance_head = this;
        return;
    }

    //链表尾插法
    CyberGearDriver_c *temp = MI_motor_instance_head;
    ...
    this->motor_can_instance_.SetRxCallBack(MiMotorReceiveCallback);
}
...
/**
  ...
  * @retval null
  */
void CyberGearDriver_c::MiMotorSetMechPosition2Zero()
{
    ...
    this->motor_can_instance_.SetExtTxId(*((uint32_t*)&(this->txMsg.ExtId)));
    for(int i = 0;i < 8;i++)
    {
        this->motor_can_instance_.tx_buff_[i] = this->txMsg.Data[i];
    }
    this->motor_can_instance_.Transmit(1);
}
...
/**
  ...
  * @retval null
  */
void CyberGearDriver_c::MiMotorEnable()
{
    this->txMsg.ExtId.mode = 3;
    this->txMsg.ExtId.motor_id = this->motor_id_; //不再重复设置
    this->txMsg.ExtId.data = this->master_id_;
    this->txMsg.ExtId.res = 0;
...
/**
  ...
  * @retval null 
  */
void CyberGearDriver_c::MiMotorGetId()
{
...
/**
  ...
  * @retval null
  */
void CyberGearDriver_c::MiMotorControlMode(float torque, float MechPosition , float speed , float kp , float kd)
{
    //断电自启动
    static int count = 0;
    if(count == 0)
...
/**
  ...
  * @retval null
  */
void CyberGearDriver_c::MiMotorStop() {
...
/**
  ...
  * @retval null
  */
void CyberGearDriver_c::MiMotorChangeID(uint8_t Target_ID)
{
    this->txMsg.ExtId.mode = 7;	
    this->txMsg.ExtId.motor_id = this->motor_id_;
    this->txMsg.ExtId.data = Target_ID << 8 | this->master_id_;
    this->txMsg.ExtId.res = 0;
    
    this->motor_id_ = Target_ID;//更改本电机canID
...
/**
  ...
  * @note   目前反馈接收存在小bug
  */
void CyberGearDriver_c::MiMotorReadOnePara( parameter_index_e index)
{
...
/**
  ...
  */
void CyberGearDriver_c::MiMotorWriteOnePara(parameter_index_e index , float Value)
{
...
/**
  ...
  * @return       : null
  */
void CyberGearDriver_c::SetMode(Mode_rum_mode_t Mode)
{
	MiMotorWriteOnePara(run_mode,Mode);
}

//速度模式控制指令
void CyberGearDriver_c::MiSpeedModeSetSpeed(float set_speed)
{
    //断电自启动
    static int count = 0;

    if(this->current_mode != speed || count == 0)
    {
        SetMode(speed);
        MiMotorEnable();
        MiMotorWriteOnePara(limit_cur,23);
        this->current_mode = speed;
    }
    count ++;
    if(count > MAX_COUNT) count = 0;
    MiMotorWriteOnePara(spd_ref,set_speed);
}

//位置模式控制指令
void CyberGearDriver_c::MiPostionModeSetPosition(float set_position)
{
    //断电自启动
    static int count = 0;

    if(this->current_mode != postion || count == 0)
    {
        SetMode(postion);
        MiMotorEnable();
        MiMotorWriteOnePara(limit_spd,5);  //最大速度，可自己另外设置
        this->current_mode = postion;
    }
    count ++;
    if(count > MAX_COUNT) count = 0;
    MiMotorWriteOnePara(loc_ref,set_position);
}

//电流模式控制指令
void CyberGearDriver_c::MiCurrentModeSetCurrent(float set_current)
{
    //断电自启动
    static int count = 0;

    if(this->current_mode != current || count == 0)
    {
        SetMode(current);
        MiMotorEnable();
    ...
    count ++;
    if(count > MAX_COUNT) count = 0;
    MiMotorWriteOnePara(iq_ref,set_current);
}
...
#endif
{
    CyberGearDriver_c* p = CyberGearDriver_c::MI_motor_instance_head;

    rx_extid = *(rxExtId_t*)&register_instance->rx_id_; //接收的扩展帧
    Rx_Data = register_instance->rx_buff_;  //接收的数据帧

    for(;p!=nullptr;p = p->next_)
    {
        CyberGearDriver_c* rx_MI_motor = p;

        if (rx_extid.motor_id != rx_MI_motor->master_id_ || rx_extid.motor_now_id != rx_MI_motor->motor_id_)  //非本主机或非本电机
        {
            continue;
        }
        ...
        switch (rx_extid.mode)
        {
            ...
            case 2:{//电机反馈数据帧,用来向主机反馈电机运行状态(通信类型2)
                ...
                rx_MI_motor->motor_data_.motor_raw_data.position=uint16_to_float(Rx_Data[0]<<8|Rx_Data[1],MIN_P,MAX_P,16);

                rx_MI_motor->motor_data_.motor_raw_data.feedback_speed=uint16_to_float(Rx_Data[2]<<8|Rx_Data[3],MIN_S,MAX_S,16);

                rx_MI_motor->motor_data_.motor_raw_data.force_feedback=uint16_to_float(Rx_Data[4]<<8|Rx_Data[5],MIN_T,MAX_T,16);

                rx_MI_motor->motor_data_.motor_raw_data.temperature[0]=(Rx_Data[6]<<8|Rx_Data[7])*Temp_Gain;
