...
class cybergear_motor_c{
    ...
    BSP_CAN_Part_n::FDCANInstance_c motor_can_ins_; // can实例
    txMsg_t            txMsg;//发送
    rxMsg_t            rxMsg;//接收
    ...
    cybergear_motor_c(FDCAN_HandleTypeDef *phcan,uint8_t master_id,uint8_t motor_id);
    ...
    friend void MI_motor_recive_callback(BSP_CAN_Part_n::FDCANInstance_c* register_instance);
};
