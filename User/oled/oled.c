...
{
    SI2C_Start();               // I2C起始
    SI2C_SendByte(0x78);        // 发送OLED的I2C从机地址
    SI2C_SendByte(0x00);        // 控制字节，给0x00，表示即将写命令
    SI2C_SendByte(Command);     // 写入指定的命令
    SI2C_Stop();                // I2C终止
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
    ...
    SI2C_Start();            // I2C起始
    SI2C_SendByte(0x78);     // 发送OLED的I2C从机地址
    SI2C_SendByte(0x40);     // 控制字节，给0x40，表示即将写数量
    /*循环Count次，进行连续的数据写入*/
    for (i = 0; i < Count; i++)
    {
        SI2C_SendByte(Data[i]); // 依次发送Data的每一个数据
    }
    SI2C_Stop(); // I2C终止
}
/**
...
void OLED_ShowChar(uint8_t X, uint8_t Y, char Char, uint8_t FontSize)
{
    ...
    else if (FontSize == OLED_16x24)
    {
        OLED_ShowImage(X, Y, 16, 24, OLED_F16X24[Char - ' ']);
    }
}
